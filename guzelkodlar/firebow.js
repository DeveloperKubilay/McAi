...
bot.on('chat', async (username, message) => {
  if (message === 'fire') {
    ...
    try {
    const nearestZombie = bot.nearestEntity(entity => entity.type === 'hostile')
    if (nearestZombie) {
      const headPosition = nearestZombie.position.offset(0, nearestZombie.height * 0.9, 0)
    } else {
      bot.chat('No zombies nearby.')
      return
    }
